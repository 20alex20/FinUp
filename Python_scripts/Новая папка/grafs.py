...
def f(time_mode: int, args, db):
    ...
    for id_category, name, description in categories:
        ...
        for id_purchase, sum, date in db.get_purchase2(id_category):
...
def f2(time_mode: int, args, db):
    ...
    while date_start < date_stop:
        ...
        for label, sum in zip(*f(time_mode, arggs, db)):
            cs[label].append(sum)
...
def graph(mode: str, time_mode: int, *args):  # year_start, month_start, day_start, year_stop, month_stop, day_stop
    db = DB("../db.sqlite3")
    print(db.login('jj', 'kk'))
    fig, ax = plt.subplots(figsize=(5, 2.7), layout='constrained')
    if mode == "plot":
        cs, labels = f2(time_mode, args, db)
        ...
        labels, sums = f(time_mode, args, db)
